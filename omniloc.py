...
def omniloc(img, xyz, rgb, input_trans, input_rot, starting_point, cfg, scalar_summaries):
    ...
    for iter in tqdm(range(num_iter), desc="Starting point {}".format(starting_point)):
        ...
        if vis:
            cur_img = Image.fromarray(make_pano(new_xyz.clone().detach(), in_rgb.clone().detach(), resolution=(img.shape[0] // 2, img.shape[1] // 2)))
    ...
    if vis:
        gt_img = Image.fromarray(np.uint8(img.detach().cpu().numpy() * 255), 'RGB').resize((img.shape[1] // 2, img.shape[0] // 2))
...
def omniloc_batch(img, xyz, rgb, input_trans, input_rot, cfg, scalar_summaries):
    # xyz, rgb are (N, 3) input point clouds
    # img is (H, W, 3) image tensor
